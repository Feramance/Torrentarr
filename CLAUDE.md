...
**Release numbering:** Torrentarr **intentionally uses a major version one ahead of qBitrr’s** (e.g. qBitrr **5**.x → Torrentarr **6**.x). Minor and patch versions are independent; the +1 major rule distinguishes the products while configs stay compatible.
...
**Config version:** Current `Settings.ConfigVersion` / `ConfigurationLoader.ExpectedConfigVersion` is **`6.14.4`**. That is **Torrentarr’s** schema id — it follows the **+1 major vs qBitrr** policy (see **What This Project Is** above), not a 1:1 copy of qBitrr’s tag. Notable fields: `HitAndRunMode` is a **string** (`"and"` / `"or"` / `"disabled"`) — not a boolean (qBitrr switched from booleans in older versions). Seeding configuration (`HitAndRunMode`, `MinSeedRatio`, `MinSeedingTimeDays`, etc.) lives in `[qBit.CategorySeeding]` per qBit instance — not in `[WebUI]`. `Settings.AutoUpdateChannel` is `"latest"` / `"stable"` / `"nightly"`. Arr sections include `[Readarr-*]`.
...
GitHub Actions runs a matrix build across Ubuntu, Windows, and macOS with .NET 10 + Node 22. Pipeline: restore → build → test (non-live) → frontend build → Docker build (on `master` push). Artifacts retained 7 days.
