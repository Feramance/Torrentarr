...
public class ConfigEndpointTests : IClassFixture<TorrentarrWebApplicationFactory>
{
    ...
    public async Task PostApiConfig_WithoutChanges_Returns400()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/config", new { });
...
public class ConfigRedactionTests : IClassFixture<LocalAuthWebApplicationFactory>
{
    ...
    /// <summary>Empty PasswordHash in POST /web/config must not clear the stored hash (account takeover bypass).</summary>
    [Fact]
