...
This guide covers performance optimization, resource usage tuning, and troubleshooting slow or resource-heavy Torrentarr deployments.

## Overview

Torrentarr is designed to be lightweight and efficient, but performance can degrade with:
...
This guide helps identify bottlenecks and optimize Torrentarr for your environment.
...
Torrentarr has multiple event loops with configurable timers:
...
**Controls:** How often Torrentarr checks torrents for completion and triggers imports.
...
**Controls:** How often Torrentarr triggers RSS feed refreshes in Arr instances.
...
!!! tip "Let Arr Handle RSS"
    If your Arr instance already has RSS configured with its own timer, you can disable Torrentarr's RSS sync:
...
**Controls:** How often Torrentarr refreshes the download queue in Arr.
...
For long-running instances (weeks/months), restart Torrentarr to clear accumulated memory:

```bash
# Cron: Weekly restart at 3 AM Sunday
0 3 * * 0 docker restart torrentarr
```
...
Place Torrentarr, qBittorrent, and Arr instances in the same Docker network:
...
# abc123...      torrentarr   5.2%      256MiB / 2GiB         12.5%
```
...
- Memory > 500 MB → Check for leaks, restart Torrentarr
...
```bash
docker logs torrentarr | grep -i delay
```
...
```bash
docker logs torrentarr | grep "API request"
```
...
3. **Check event loop delays:**
   ...
   ```bash
   docker logs torrentarr | tail -100 | grep -i delay
   ```
...
Torrentarr uses `pathos.multiprocessing` to run each Arr instance in a separate process:
...
Lower Torrentarr's process priority:
