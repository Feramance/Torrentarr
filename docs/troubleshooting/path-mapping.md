...
This guide covers path mapping issues between qBittorrent, Arr instances (Radarr/Sonarr/Lidarr), and Torrentarr, focusing on Docker volume mapping and file accessibility.

## Overview

Path mapping is one of the most common sources of confusion and errors in Torrentarr deployments. When qBittorrent, Arr instances, and Torrentarr run in separate containers (or a mix of containers and host installs), they each see the file system differently.
...
4. Torrentarr can't find the file at `/completed` when it tries to verify
...
## Torrentarr Configuration

### CompletedDownloadFolder

In `config.toml`, set the path where qBittorrent saves completed downloads **as seen by Torrentarr's container**:
...
Torrentarr automatically detects category-specific save paths from qBittorrent's API.
...
- qBittorrent, Radarr, Sonarr, Torrentarr all in Docker on one server
...
- Torrentarr runs natively (systemd, not Docker)
...
**Why this works:** Native Torrentarr sees the host file system directly, while containers see `/data` → `/mnt/storage` mapping.
...
| Torrentarr (Native) | `/mnt/storage/torrents/Movie.mkv` | `/mnt/storage/torrents/Movie.mkv` |
...
- ❌ Torrentarr may not see files at expected paths
...
2. **Path Not Found Errors**
   ```
   [Torrentarr] FileNotFoundError: /data/torrents/Movie.mkv
   ```

3. **Instant Import Not Triggering**
   ```
   [Torrentarr] Torrent completed but no import triggered
   ```
...
# Inspect Torrentarr container
docker inspect torrentarr | grep -A 10 Mounts
```
...
# Check Torrentarr sees it
docker exec torrentarr ls -lh /data/torrents/test.txt
...
| Torrentarr | `/mnt/storage/torrents` | `/completed` |
...
docker exec torrentarr ls /data/torrents/test.txt
...
3. **Check Torrentarr sees the download**
   ```bash
   docker exec torrentarr ls -lh /data/torrents/
   ```

4. **Monitor Torrentarr logs**
   ```bash
...
TEST_FILE="torrentarr-path-test-$(date +%s).txt"
...
echo "Testing Torrentarr..."
docker exec torrentarr ls -lh "$CONTAINER_PATH/$TEST_FILE" || echo "❌ Torrentarr FAIL"
...
- Torrentarr triggers instant import
...
- Torrentarr may not detect completion
...
3. **Fallback to copies:**
   ...
   If atomic moves fail, Radarr will copy. Torrentarr will still work but imports will be slower.
