...
docker run --rm -v /path/to/config:/config feramance/torrentarr:latest --gen-config

# Or create directory and let Torrentarr generate it
...
**Symptom:** qBittorrent is running on host, Torrentarr in Docker can't connect
...
### Torrentarr Can't See Completed Downloads

**Symptom:** Torrents complete but Torrentarr doesn't trigger import

**Check volume mapping:**

```bash
# List files inside Torrentarr container
docker exec torrentarr ls -la /data/torrents
...
```bash
docker exec torrentarr printenv | grep -i qbit
```
...
docker logs --tail 100 torrentarr
...
# Last 50 lines
docker logs --tail 50 torrentarr
...
docker exec torrentarr ls -la /config
...
docker exec -it torrentarr /bin/sh

# Or bash if available
docker exec -it torrentarr /bin/bash

# Check environment variables
docker exec torrentarr printenv
...
docker exec torrentarr curl -v http://qbittorrent:8080/api/v2/app/version

# From container to Radarr
docker exec torrentarr curl -H "X-Api-Key: your-api-key" http://radarr:7878/api/v3/system/status
```
...
docker-compose down torrentarr
docker pull feramance/torrentarr:5.1.0  # Specific version
docker-compose up -d torrentarr
```
...
services:
  torrentarr:
    image: feramance/torrentarr:5.1.0  # Pin to specific version
```
...
tar -czf torrentarr-backup-$(date +%Y%m%d).tar.gz ./torrentarr/config/

# Or just the config file
cp ./torrentarr/config/config.toml torrentarr-config-backup.toml
```
...
```bash
# Extract backup
tar -xzf torrentarr-backup-20250326.tar.gz

# Or copy config
cp torrentarr-config-backup.toml ./torrentarr/config/config.toml
