...
Torrentarr provides extensive logging capabilities to help diagnose issues:
...
|-------|----------|---------------|
| `CRITICAL` | Production | Only fatal errors that stop Torrentarr |
...
```
~/config/logs/
├── Main.log              # Core Torrentarr operations
...
docker exec torrentarr ls -lh /config/logs
docker exec torrentarr tail -f /config/logs/Main.log
```
...
```
2025-11-27 12:34:56,789 | INFO     | Torrentarr.arr.Radarr-4K | Processing torrent: Movie.2024.1080p.mkv [hash123abc]
```
...
3. **Logger**: `Torrentarr.arr.Radarr-4K` (module.submodule.instance)
4. **Message**: Descriptive log message

### Logger Hierarchy

```
Torrentarr                    # Root logger
├── Torrentarr.arr            # Arr management
│   ├── Torrentarr.arr.Radarr-4K
│   ├── Torrentarr.arr.Sonarr-TV
│   └── Torrentarr.arr.Lidarr
├── Torrentarr.webui          # WebUI server
├── Torrentarr.config         # Configuration loading
├── Torrentarr.ffprobe        # Media validation
└── Torrentarr.auto_update    # Auto-update system
```
...
=== "Docker"
    ...
    # Multiple files
    docker exec torrentarr tail -f /config/logs/*.log
    ```
...
#### Torrent Processing
```
INFO | Torrentarr.arr.Radarr-4K | Processing torrent: Movie.2024.1080p.mkv [hash123]
DEBUG | Torrentarr.arr.Radarr-4K | Torrent status: downloading, progress: 45.2%
DEBUG | Torrentarr.arr.Radarr-4K | ETA: 1234 seconds, peers: 15
INFO | Torrentarr.arr.Radarr-4K | Torrent completed: Movie.2024.1080p.mkv
```

#### Instant Import
```
INFO | Torrentarr.arr.Radarr-4K | Triggering instant import for: Movie.2024.1080p.mkv
DEBUG | Torrentarr.arr.Radarr-4K | POST /api/v3/command {"name": "DownloadedMoviesScan", "path": "/downloads/Movie.2024.1080p.mkv"}
INFO | Torrentarr.arr.Radarr-4K | Import command sent successfully
```

#### Search Operations
```
INFO | Torrentarr.arr.Radarr-4K | Starting missing media search
DEBUG | Torrentarr.arr.Radarr-4K | Found 12 missing movies
DEBUG | Torrentarr.arr.Radarr-4K | Searching for: "Movie Title (2024)"
DEBUG | Torrentarr.arr.Radarr-4K | POST /api/v3/command {"name": "MoviesSearch", "movieIds": [123]}
INFO | Torrentarr.arr.Radarr-4K | Search completed: 12 items searched, 5 found
```
...
#### Connection Failures
```
ERROR | Torrentarr.arr.Radarr-4K | Connection refused: http://radarr:7878/api/v3/system/status
ERROR | Torrentarr.arr.Radarr-4K | Failed to connect to Radarr instance
WARNING | Torrentarr.arr.Radarr-4K | Will retry in 30 seconds
```
...
```
ERROR | Torrentarr.qbit | qBittorrent login failed: Invalid username or password
ERROR | Torrentarr.qbit | Authentication error, check credentials in config.toml
```
...
#### API Key Errors
```
ERROR | Torrentarr.arr.Radarr-4K | HTTP 401: Unauthorized
ERROR | Torrentarr.arr.Radarr-4K | API key is invalid or missing
```
...
#### Path Mapping Issues
```
ERROR | Torrentarr.arr.Radarr-4K | Completed download folder not accessible: /downloads
ERROR | Torrentarr.arr.Radarr-4K | FileNotFoundError: [Errno 2] No such file or directory: '/downloads/Movie.2024.1080p.mkv'
```

**Diagnosis**: Path mismatch between qBittorrent, Arr, and Torrentarr. Verify Docker volumes or native paths.

#### Database Errors
```
ERROR | Torrentarr | Database is locked
ERROR | Torrentarr | OperationalError: database is locked
WARNING | Torrentarr | Retrying database operation (attempt 2/5)
```

**Diagnosis**: Multiple Torrentarr instances or file permissions issue. Check for duplicate processes.
...
Torrentarr automatically rotates logs when they reach **10 MB**:
...
# Restart Torrentarr (creates new logs)
...
tar -czf torrentarr-logs-$(date +%Y%m%d-%H%M%S).tar.gz \
  ~/config/logs/*.log \
  ~/config/config.toml
...
2. Torrentarr is actually running:
   ```bash
   docker ps | grep torrentarr  # Docker
   systemctl status torrentarr  # Systemd
   ```

3. Check stdout/stderr instead:
   ```bash
   docker logs torrentarr  # Docker
   journalctl -u torrentarr  # Systemd
   ```
...
2. Implement external log rotation:
   ...
   sudo nano /etc/logrotate.d/torrentarr
   ```
