...
| `qBitrr/arss/` (`arr_base.py`, `torrent_*.py`, `search_handlers.py`, `db_update_handlers.py`, `db_queries.py`, `request_providers.py`, `placeholder_arr.py`, type modules, `manager.py`, `factory.py`, `qbit_side_effects.py`) | `ArrWorkerManager`, `TorrentProcessor`, `ArrSyncService`, `SearchExecutor`, `ArrMediaService`, `ArrImportService`, `OverseerrRequestFetcher` | full | Dual **in-process** torrent/search tasks (not pathos OS forks). `SearchMissing` is the search-loop master switch. Explicit `loop_completed` resets `Searched` and `Upgrade`. Lidarr `QualityMet` from `percentOfTracks` + cutoff/`upgradeAllowed`. Import completion waits for Arr confirmation. Multi-instance routing via `torrent.QBitInstanceName`. **Evidence:** [`ArrWorkerManagerTests`](https://github.com/Feramance/Torrentarr/blob/master/tests/Torrentarr.Infrastructure.Tests/Services/ArrWorkerManagerTests.cs), [`TorrentProcessorTests`](https://github.com/Feramance/Torrentarr/blob/master/tests/Torrentarr.Infrastructure.Tests/Services/TorrentProcessorTests.cs). |
| `qBitrr/config_reload_policy.py` | `ConfigReloader` | full | Hot-reload from disk; dependent services (including Arr workers) restart on change. |
| `qBitrr/process_lifecycle.py` | `ArrWorkerManager`, `ProcessStateManager` | full | User-facing restart limits match qBitrr; implemented as in-process task restart rather than OS-process restart. |
| `qBitrr/quality_profile_helpers.py` | `QualityProfileSwitcherService` | full | Temp profiles on Radarr, Sonarr, Lidarr, and Readarr. |
| `qBitrr/radarr_availability.py` | `MinimumAvailabilityCheck` | full | Radarr cinema/digital/physical/announced windows. |
| `qBitrr/qbit_seeding_config.py` | `SeedingLimitMerge`, per-instance `CategorySeeding` / `Trackers` | full | `-1` merge only when no source sets a positive limit. |
| `qBitrr/arr_client.py` | `ApiClients/Arr/*.cs` | full | Radarr/Sonarr/Lidarr/Readarr HTTP clients. |
...
| `qBitrr/ffprobe.py` | `MediaValidationService.cs`, `TorrentProcessor` post-import cleanup | full | After `Downloaded*Scan`, if `Torrent.AutoDelete`, probe allowlisted files; zero valid media → Arr queue delete with blacklist + delete local files. Missing ffprobe and ebook/comic suffixes count as valid. Host DI + optional `FFprobeAutoUpdate`. |
...
| `scripts/repair_database_targeted.py` | Host `--repair-database`, `DatabaseHealthService.RepairAsync` | intentional-divergence | Same user outcome (recover a corrupt catalog) via SQLite backup-restore; table-scoped Python script is not cloned. See [contributor-reference.md](contributor-reference.md#targeted-database-repair). |
