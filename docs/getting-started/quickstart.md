...
- Torrentarr installed ([installation guide](installation/index.md))

## Step 1: Start Torrentarr
...
On first run, Torrentarr will generate a default `config.toml` file.

## Step 2: Stop Torrentarr

Stop Torrentarr so you can edit the configuration:
...
Torrentarr **requires** your Arr downloads to be tagged so it knows which torrents to monitor.
...
3. Watch Torrentarr logs – it should detect the new torrent
4. When the download completes, Torrentarr will trigger an import
...
```bash
# From Torrentarr container/host
...
**Symptoms:** Torrentarr runs but doesn't detect torrents
...
1. **Let Torrentarr download FFprobe:**
   ```toml
   [Settings]
   FFprobeAutoUpdate = true
   FFprobeAutoUpdate = true
...
**Symptoms:** Torrentarr using excessive CPU
...
**Problem:** Radarr uses `radarr`, but Torrentarr config has `radarr-movies`

**Solution:** Ensure categories match exactly between your Arr download client configuration and Torrentarr config.

### Missing Tags

**Problem:** Torrents have tags but Torrentarr has `Tagless = false`
...
Access your Torrentarr WebUI at http://localhost:6969/ui to:
...
1. **Check logs for errors:**
   ```bash
   docker logs torrentarr | grep ERROR
   ```
...
- 💬 [GitHub Discussions](https://github.com/Feramance/Torrentarr/discussions) - Community support
- 🐛 [GitHub Issues](https://github.com/Feramance/Torrentarr/issues) - Bug reports
...
3. **Relevant logs:**
   ```bash
   docker logs torrentarr 2>&1 | tail -100
   ```
...
**Congratulations!** 🎉 You've successfully set up Torrentarr. Happy automating!
