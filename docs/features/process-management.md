...
1. **Process Failure Detection**: Torrentarr detects when an Arr process terminates unexpectedly
2. **Restart Attempt**: If `AutoRestartProcesses = true`, Torrentarr waits `ProcessRestartDelay` seconds
3. **Restart Count Tracking**: Restart timestamp is recorded for the process
4. **Window Check**: Torrentarr checks if restarts exceed `MaxProcessRestarts` within `ProcessRestartWindow`
5. **Decision**:
...
```bash
# Docker
docker exec torrentarr ps aux | grep "Torrentarr"
...
Torrentarr automatically restarts processes when:
...
**Via Docker:**
```bash
# Restart entire Torrentarr container
docker restart torrentarr
```

**Via systemd:**
```bash
# Restart Torrentarr service
...
Torrentarr implements graceful shutdown to ensure data consistency:
...
3. ✅ Manually restart Torrentarr after fixing:
   ```bash
...
**Symptoms:** Torrentarr consuming excessive RAM
...
1. ✅ Monitor per-process memory:
   ```bash
   docker stats torrentarr  # Docker
...
4. **Use External Health Checks**: Monitor Torrentarr's WebUI endpoint (`/api/health`) with Uptime Kuma, Healthchecks.io, etc.
...
- **Torrentarr Auto-Restart**: Restarts individual Arr processes
...
**Torrentarr Service (`torrentarr.service`):**
```ini
[Unit]
Description=Torrentarr - qBittorrent and Arr Integration
After=network.target

[Service]
Type=simple
User=torrentarr
WorkingDirectory=/home/torrentarr
...
**Combined with Torrentarr Auto-Restart:**
- systemd restarts Torrentarr if main process crashes
- Torrentarr restarts individual Arr processes if they crash
...
**Prometheus/Grafana:**
- Scrape Torrentarr metrics endpoint (future feature)
- Alert on process restart count

**Uptime Kuma:**
```yaml
# Monitor Torrentarr WebUI
- name: Torrentarr
  type: HTTP
...
**Healthchecks.io:**
```bash
# Cron job to ping healthchecks.io if Torrentarr is running
...
**Workaround:** Run multiple Torrentarr instances (one per Arr) with different restart settings.
