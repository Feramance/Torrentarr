...
Torrentarr automatically monitors disk space and pauses downloads when free space drops below your configured threshold, preventing "disk full" errors and system instability.
...
```mermaid
graph TD
    A[Torrentarr monitoring loop] --> B{Check free space}
    B --> C{Space < FreeSpace?}
...
1. Torrentarr checks free space across **all configured qBittorrent instances** every `LoopSleepTimer` seconds (default: 5)
2. Compares available space (per-instance `DownloadPath` drive) to `FreeSpace` threshold
3. If space < threshold on any instance:
   - Gathers downloading torrents from ALL instances, sorted globally by `AddedOn` date (oldest first)
   - Pauses the oldest downloading torrents until enough space is reclaimed
   - Keeps seeding torrents active
   - Logs warning message
4. When space frees up (files imported/deleted):
   - Automatically resumes paused downloads across all instances
...
- Torrentarr can pause downloads when disk is full
- Torrentarr can resume downloads when space frees up
...
# Docker
docker logs -f torrentarr | grep -i "space\|pause\|resume"
```
...
3. Check if they were auto-paused by Torrentarr
...
**Symptom:** Torrentarr reports plenty of space, but disk is full
...
Use external scripts with Torrentarr:
