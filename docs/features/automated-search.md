...
Torrentarr can automatically search your Arr instances for missing media and quality upgrades, eliminating manual intervention for building and maintaining your library.

---

## Overview

Instead of manually clicking "Search All Missing" in Radarr/Sonarr/Lidarr, Torrentarr continuously searches for:
...
```mermaid
graph TD
    A[Torrentarr starts] --> B[Update Torrentarr database from Arrs]
    B --> C{Items found?}
...
1. Torrentarr queries Arr API for missing media
...
That's the minimum! Torrentarr will now continuously search for missing movies with a 30-second delay between each search command.
...
**Master switch for ALL search functionality in Torrentarr.**

When `true`:

- Torrentarr searches for missing media
...
- Torrentarr only manages existing torrents (no new searches)
...
Controls how frequently Torrentarr checks for new requests from Overseerr/Ombi and triggers searches.

**What it does:**
- After processing all pending requests, Torrentarr waits this many seconds before checking for new requests again
...
!!! warning "This ERASES Search History"
    ...
    When enabled, Torrentarr marks **ALL items as `Searched=False`** after each search loop completion, causing them to be re-searched from scratch.

**How It Works:**

1. Torrentarr searches all missing/upgrade items
...
1. Torrentarr extracts all release years from your content (e.g., `[2020, 2021, 2022, 2023, 2024]`)
...
1. Torrentarr checks for episodes airing today
...
Torrentarr queries the Arr's `/api/v3/command` endpoint to count **active search tasks running in Radarr/Sonarr**. If the count reaches `SearchLimit`, Torrentarr **pauses** sending new searches until the queue clears.

**What this controls:**
- ✅ Depth of Radarr/Sonarr's internal task queue
- ✅ How many searches Torrentarr allows to pile up in Arr

**What this does NOT control:**
- ❌ Number of torrents Torrentarr searches simultaneously
- ❌ Number of parallel API calls Torrentarr makes
- ❌ Queue size in Torrentarr's database

**Platform-Specific Limits:**

- **Sonarr:** Hardcoded limit of 3 concurrent tasks. Setting `SearchLimit > 3` allows Torrentarr to queue additional searches that Sonarr will process sequentially.
...
- Torrentarr queries for all media (not just missing)
...
3. Torrentarr detects quality not met
...
!!! danger "CRITICAL: DoUpgradeSearch Overrides Quality/CF Searches"
    ...
    **`DoUpgradeSearch` takes precedence over `QualityUnmetSearch` and `CustomFormatUnmetSearch`.**
    ...
    - Torrentarr searches **ALL items with files** for potential upgrades
    - `QualityUnmetSearch` and `CustomFormatUnmetSearch` settings are **IGNORED**

    When `DoUpgradeSearch=false`:
    - Torrentarr respects `QualityUnmetSearch` and `CustomFormatUnmetSearch`
...
3. Torrentarr triggers search for higher-scoring release
...
- Torrentarr checks custom format scores
...
2. Torrentarr switches to "Any (MP3 acceptable)"
3. Lidarr finds and downloads MP3
4. Torrentarr switches back to "Lossless"
...
Reset all items using temp profiles back to main profiles on Torrentarr startup.
...
After 7 days with temp profile, Torrentarr resets to main profile.
...
**Symptom:** Torrentarr doesn't search for missing items
...
3. **Manual batch search in Arr:**
   ...
   - Then enable Torrentarr for maintenance
...
- `ForceResetTempProfiles=true` triggers on Torrentarr restart
...
Sonarr only supports quality profiles at the **series level** (not episode level). All episodes inherit the series profile. This is a Sonarr limitation, not a Torrentarr design choice.
...
**A:** Torrentarr handles this gracefully:

1. Profile deleted in Arr
2. Torrentarr logs warning: "Profile 'ProfileName' not found"
