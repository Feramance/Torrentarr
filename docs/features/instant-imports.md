...
Torrentarr's instant import feature triggers media imports in your Arr instances immediately upon download completion, eliminating the wait for periodic scans.

---

## Overview

Traditional workflow (without Torrentarr):
...
With Torrentarr's instant imports:
...
```mermaid
graph TD
    A[qBittorrent completes download] --> B{Torrentarr Loop Check}
...
Torrentarr checks torrent status every `LoopSleepTimer` seconds (default: 5):
...
Torrentarr sends an appropriate API command to Arr based on the media type:
...
**With Torrentarr (LoopSleepTimer=5):**

- Download completes: `00:00:00`
- Torrentarr detects: `00:00:05` (max)
...
- Torrentarr monitors torrents continuously
...
3. **Torrentarr imports instantly**
...
Instant imports are **automatic** when Torrentarr manages an Arr instance. No special configuration needed.

### Basic Setup

```toml
[Radarr-Movies]
# Enable Torrentarr management
Managed = true
...
- Torrentarr validates media files before triggering import
...
sequenceDiagram
    ...
    loop Every LoopSleepTimer seconds
        Torrentarr->>qBT: Check torrent status
        alt Still downloading
            qBT-->>Torrentarr: Progress: 45%, Speed: 5 MB/s
            Note over Torrentarr: Continue monitoring
        end
    end
    ...
    Torrentarr->>qBT: Check status (next loop)
    qBT-->>Torrentarr: State: seeding, Progress: 100%

    alt FFprobe Enabled
        Torrentarr->>FS: Read media file
        Torrentarr->>FFprobe: Validate codec/duration

        alt File Valid
            FFprobe-->>Torrentarr: ✅ Valid MKV, 2h 16m, H.264
            Note over Torrentarr: Proceed to import
        else File Corrupt
            FFprobe-->>Torrentarr: ❌ Corruption detected
            Torrentarr->>Arr: POST /api/v3/queue/blacklist
            Torrentarr->>qBT: Delete torrent + files
            Torrentarr->>Arr: POST /api/v3/command (MoviesSearch)
            Note over Arr: Search for alternative
        end
    end

    Torrentarr->>Arr: POST /api/v3/command<br/>{"name": "DownloadedMoviesScan"}
...
1. **Monitoring Loop**: Torrentarr polls qBittorrent every `LoopSleepTimer` seconds
...
5. **Torrentarr detects completion** (within 5 seconds)
6. Torrentarr validates file with FFprobe (2-3 seconds)
7. **Torrentarr triggers Radarr import**
...
4. Download completes
5. **Torrentarr detects completion**
...
4. Download completes
5. **Torrentarr detects completion**
6. Torrentarr validates all 11 FLAC files
7. **Torrentarr triggers Lidarr import**
...
1. Torrentarr marks torrent as failed
...
4. **Torrentarr imports instantly**
...
- :material-close-circle:{ .lg .middle style="color: #ff6b6b" } **Without Torrentarr**
...
- :material-check-circle:{ .lg .middle style="color: #51cf66" } **With Torrentarr**
...
| Metric | Without Torrentarr | With Torrentarr | Savings |
|:-------|---------------:|------------:|--------:|
| Search & select | 5s | 5s | - |
...
**Symptom:** Torrentarr detects but import takes a long time
...
**Symptom:** Torrentarr triggers import but Arr rejects it
...
## Comparison: With vs Without Torrentarr
...
sequenceDiagram
    ...
    rect rgb(255, 243, 191)
        Note over User,Arr: Traditional Setup (No Torrentarr)
    ...
    rect rgb(211, 249, 216)
        Note over User,Torrentarr: With Torrentarr (Instant Import)
        User->>Arr: Request movie
        Arr->>Arr: Search (30s)
        Arr->>qBT: Send torrent
        qBT->>qBT: Download (400s)
        qBT->>qBT: Complete ✅
        Torrentarr->>qBT: ⚡ Detect completion (5s)
        Torrentarr->>Torrentarr: FFprobe validate (3s)
        Torrentarr->>Arr: Trigger import NOW (1s)
        Arr->>Arr: Import (4s)
        Arr-->>User: Movie available
        Note over User,Torrentarr: TOTAL: 443s (7m 23s)
    end
```
...
- :material-lightning-bolt:{ .lg .middle style="color: #51cf66" } **Torrentarr Setup**
