...
Torrentarr uses qBittorrent categories to track which Arr instance owns each torrent:
...
Torrentarr can trigger imports immediately when downloads complete:
...
**Problem:** Torrentarr can't connect to Arr instance
...
**Solutions:**
- Verify category in Torrentarr matches Arr download client category
...
This allows Torrentarr to temporarily lower quality requirements to get *something* now, then upgrade when better releases appear.
...
Configure when and how often Torrentarr searches for missing content:
...
4. Torrentarr searches for and downloads
5. Torrentarr imports to Radarr
...
Filter what content Torrentarr searches for:
...
**Solutions:**
- Verify URI is correct and accessible from Torrentarr container/host
...
**Cause:** Category in Torrentarr doesn't match Arr download client category

**Solution:**
1. Check Torrentarr category: `Category = "radarr"` in config
...
3. **Instant Import Disabled**
   ...
   - Fix: Enable `InstantImport = true` in Torrentarr config
