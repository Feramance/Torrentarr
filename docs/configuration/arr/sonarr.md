...
This guide covers how to configure Sonarr instances in Torrentarr for TV show management, automated searching, episode tracking, and quality upgrades.

---

## Quick Start

Every Sonarr instance in Torrentarr requires a dedicated section in your `config.toml` file. The section name must follow the pattern `Sonarr-<name>`.
...
!!! warning "Category Mismatch"
    The `Category` value in Torrentarr **must exactly match** the category configured in Sonarr's qBittorrent download client settings. If they don't match, Torrentarr won't process your Sonarr torrents.
...
When `Managed = false`, Torrentarr will completely ignore this Sonarr instance. Useful for temporarily disabling an instance without removing its configuration.
...
When enabled, Torrentarr automatically triggers a new search in Sonarr when:
...
!!! info "Sonarr's Built-in RSS"
    Sonarr has its own RSS sync schedule. Torrentarr's `RssSyncTimer` is **additional** to Sonarr's built-in sync, providing extra responsiveness for new episode releases.
...
When Sonarr encounters these error messages during import, Torrentarr will:
...
# Maximum concurrent searches Torrentarr will queue (Sonarr hardcoded cap: 3)
SearchLimit = 5
...
!!! warning "Sonarr Task Limit"
    Sonarr has a **hardcoded limit of 3 simultaneous tasks**. Setting `SearchLimit` higher than 3 won't increase actual concurrency, but it does affect Torrentarr's search queue management.
...
When enabled, Torrentarr searches for episodes that aired **today** before processing the general missing episode queue. This ensures new episodes are grabbed quickly after they air.
...
# Reset all temp profiles on Torrentarr startup
...
2. Torrentarr switches it to `Web-DL 1080p` profile (temp)
3. Sonarr searches with lower quality requirements
4. After import, Torrentarr switches back to `Ultra-HD` (main)
...
!!! info "4K Instance Detection"
    Set `Is4K = true` if your Sonarr instance is configured as the 4K instance in Overseerr. This ensures Torrentarr pulls the correct requests.
...
Configure how Torrentarr handles Sonarr's torrents in the `[Sonarr-TV.Torrent]` subsection. See the [Torrent Settings](../torrents.md) page for comprehensive documentation.
...
**Symptoms:** Torrentarr doesn't monitor or manage Sonarr torrents

**Solutions:**

1. ✅ Verify `Category` in Torrentarr matches Sonarr's download client category exactly
...
**Symptoms:** Torrentarr doesn't search for missing episodes
...
5. ✅ Check Torrentarr logs for search activity
