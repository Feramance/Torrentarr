...
This guide covers how to configure Lidarr instances in Torrentarr for music library management, automated searching, and quality upgrades.

---

## Quick Start

Every Lidarr instance in Torrentarr requires a dedicated section in your `config.toml` file. The section name must follow the pattern `Lidarr-<name>`.
...
# If true, do not verify TLS for this Servarr API (HTTPS).
SkipTLSVerify = false
...
!!! warning "Category Mismatch"
    The `Category` value in Torrentarr **must exactly match** the category configured in Lidarr's qBittorrent download client settings. If they don't match, Torrentarr won't process your Lidarr torrents.
...
When `Managed = false`, Torrentarr will completely ignore this Lidarr instance. Useful for temporarily disabling an instance without removing its configuration.
...
When enabled, Torrentarr automatically triggers a new search in Lidarr when:
...
!!! info "Lidarr's Built-in RSS"
    Lidarr has its own RSS sync schedule. Torrentarr's `RssSyncTimer` is **additional** to Lidarr's built-in sync, providing extra responsiveness for new releases.
...
When Lidarr encounters these error messages during import, Torrentarr will:
...
2. Torrentarr switches it to `MP3-320` profile (temp)
3. Lidarr searches with lower quality requirements
4. After import, Torrentarr switches back to `Lossless` (main)
...
Configure how Torrentarr handles Lidarr's torrents in the `[Lidarr-Music.Torrent]` subsection.
...
**Symptoms:** Torrentarr doesn't monitor or manage Lidarr torrents

**Solutions:**

1. ✅ Verify `Category` in Torrentarr matches Lidarr's download client category exactly
...
**Symptoms:** Torrentarr doesn't search for missing albums
