...
This guide covers how to configure Radarr instances in Torrentarr for movie management, automated searching, and quality upgrades.

---

## Quick Start

Every Radarr instance in Torrentarr requires a dedicated section in your `config.toml` file. The section name must follow the pattern `Radarr-<name>`.
...
!!! warning "Category Mismatch"
    The `Category` value in Torrentarr **must exactly match** the category configured in Radarr's qBittorrent download client settings. If they don't match, Torrentarr won't process your Radarr torrents.
...
When `Managed = false`, Torrentarr will completely ignore this Radarr instance. Useful for temporarily disabling an instance without removing its configuration.
...
When enabled, Torrentarr automatically triggers a new search in Radarr when:
...
!!! info "Radarr's Built-in RSS"
    Radarr has its own RSS sync schedule. Torrentarr's `RssSyncTimer` is **additional** to Radarr's built-in sync, providing extra responsiveness.
...
When Radarr encounters these error messages during import, Torrentarr will:
...
# Maximum concurrent searches Torrentarr will queue (Radarr default limit: 3, max: 10)
SearchLimit = 5
...
!!! info "4K Instance Detection"
    Set `Is4K = true` if your Radarr instance is configured as the 4K instance in Overseerr. This ensures Torrentarr pulls the correct requests.
...
Configure how Torrentarr handles Radarr's torrents in the `[Radarr-Movies.Torrent]` subsection. See the [Torrent Settings](../torrents.md) page for comprehensive documentation.
...
**Symptoms:** Torrentarr doesn't monitor or manage Radarr torrents

**Solutions:**

1. ✅ Verify `Category` in Torrentarr matches Radarr's download client category exactly
...
**Symptoms:** Torrentarr doesn't search for missing movies
