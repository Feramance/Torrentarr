...
Torrentarr connects to qBittorrent's Web UI to monitor and manage torrents. This page covers all qBittorrent-related settings and how to configure the connection.
...
4. Check the following:
   - **IP Address** – Use this for `Host` (use `localhost` or `127.0.0.1` if Torrentarr is on the same machine)
...
|---------|---------|-------|
| `Host` | `localhost` | Same machine as Torrentarr |
...
Torrentarr can run in "headless mode" where it **only processes searches** without managing qBittorrent torrents. This is useful if you use a different download client (like Sabnzbd/NZBGet) but still want Torrentarr's automated search features.
...
!!! tip "When to Use Headless Mode"
    ...
    - You still want Torrentarr's faster automated search features
    - You have a separate qBittorrent setup you don't want Torrentarr to touch
...
If qBittorrent and Torrentarr are in the same Docker network, use the **container name** as the host:
...
You can test if Torrentarr can reach qBittorrent's API:
...
1. ✅ Restart Torrentarr to establish a new session
...
!!! success "New in v5.7.0"
    Torrentarr now supports managing torrents across **multiple qBittorrent instances** simultaneously! This enables load balancing, redundancy, VPN isolation, and more.

### Overview

With multi-instance support, you can configure multiple qBittorrent instances and Torrentarr will:
...
- Torrentarr monitors ALL instances for torrents in the `radarr-movies` category
- If a Radarr torrent appears on `default` or `seedbox`, Torrentarr manages it the same way

### Configuration Syntax

All qBittorrent instances are treated equally — there is no mandatory "primary" instance. By convention `[qBit]` is the name for the main instance, but you can name them anything that follows the `[qBit]` or `[qBit-NAME]` pattern:

```toml
[qBit]  # Main instance (conventional name, not special in code)
Host = "localhost"
Port = 8080
UserName = "admin"
Password = "adminpass"

[qBit-seedbox]  # Second instance (same priority as the first)
Host = "192.168.1.100"
Port = 8080
UserName = "admin"
Password = "seedboxpass"

[qBit-vpn]  # Third instance
Host = "10.8.0.2"
Port = 8080
UserName = "admin"
Password = "vpnpass"
...
!!! warning "Important: Use Dash Notation"
    All instances MUST use dash (`-`) notation, NOT dot (`.`) notation:

    - ✅ **Correct:** `[qBit]`, `[qBit-seedbox]`, `[qBit-vpn]`, `[qBit-remote]`
...
Torrentarr's WebUI provides a graphical interface for managing multiple qBittorrent instances:
...
4. Restart Torrentarr to activate the new instance
...
Torrentarr automatically monitors the health of each instance and handles failures gracefully:
...
1. ✅ Verify category exists on all instances (Torrentarr creates them automatically)
...
**Step 3:** Restart Torrentarr
```bash
systemctl restart torrentarr  # OR docker restart torrentarr
```
...
Torrentarr can manage qBittorrent categories independently of Radarr/Sonarr/Lidarr. This is useful for torrents that aren't tracked by any Arr instance, such as manually added downloads, private tracker torrents, or long-term seeds.

### Overview

When you add categories to the `ManagedCategories` list, Torrentarr spawns a dedicated worker process that:
...
| `MaxSeedingTime` | `-1` | seconds | Maximum seeding duration before removal action. `-1` disables. Met when qBittorrent `seeding_time` reaches this value, **or** when Torrentarr has observed a stalled upload (`stalledUP`) for this long. The stalled clock is not `last_activity`; queue/pause time does not count, and a restart resets the window. HnR still uses actual `seeding_time` and can block deletion. |
...
!!! note "Torrent Removal"
    When Torrentarr removes a torrent, the torrent entry is removed from qBittorrent but **downloaded files are preserved** on disk (`delete_files=False`).
...
!!! info "Certificate Validation"
    Torrentarr validates SSL certificates by default. Self-signed certificates may cause connection errors. Consider using a proper certificate or a reverse proxy with valid TLS.
