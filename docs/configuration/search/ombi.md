...
!!! tip "Requestrr Integration"
    ...
    then Torrentarr will automatically process those requests.
    [**→ Requestrr Integration Guide**](requestrr.md)

Ombi is a self-hosted request management system for Plex, Emby, and Jellyfin. Torrentarr can integrate with Ombi to automatically search for user-requested content, ensuring requests are fulfilled quickly and efficiently.

## Overview

When Ombi integration is enabled, Torrentarr will:
...
graph LR
    ...
    B --> C[Torrentarr Polls Ombi API]
    C --> D[Torrentarr Filters by Approval Status]
    D --> E[Torrentarr Triggers Search in Arr]
    E --> F[Arr Searches Indexers]
    F --> G[Torrent Downloaded to qBittorrent]
    G --> H[Torrentarr Monitors & Imports]
...
4. Copy the API key
5. Paste it into your Torrentarr config as `OmbiAPIKey`
...
# Direct
# Stop and restart the torrentarr process
```
...
```bash
# Docker
docker logs -f torrentarr 2>&1 | grep -i ombi
...
Torrentarr will **only** search for requests that have been:
- Approved by an admin in Ombi
- Not denied
- Not already available in your library

**Request States Processed:**
- ✅ **Approved** → Torrentarr triggers search in Arr
...
Torrentarr will search for **any** requests that are:
- Not denied
- Not already available in your library

**Request States Processed:**
- ✅ **Approved** → Torrentarr triggers search
- ✅ **Pending** → Torrentarr triggers search
- ❌ **Denied** → Ignored (explicitly rejected)
- ❌ **Available** → Ignored (already in library)

!!! warning "Automatic Approval"
    Setting `ApprovedOnly = false` effectively **auto-approves all requests**, as Torrentarr will search for them immediately. Use with caution, especially on public instances.
...
!!! tip "API Key Reuse"
    Multiple Arr instances can use the **same Ombi API key**. Torrentarr filters requests based on the Arr type (movies vs TV shows).
...
4. ✅ Test API key manually:
   ...
   ```
5. ✅ Check Torrentarr logs for Ombi errors:
   ```bash
   tail -f ~/logs/Radarr-Movies.log | grep -i ombi
   ```
...
5. ✅ Check Docker network connectivity (if using containers):
   ```bash
   docker exec torrentarr ping ombi
   ```
...
**Symptoms:** Torrentarr searches for requests marked as "Denied" in Ombi
...
4. ✅ Restart Torrentarr to refresh request cache:
   ```bash
   docker restart torrentarr  # Docker
   sudo systemctl restart torrentarr  # systemd
   ```
...
**Symptoms:** Requests approved in Ombi, but Torrentarr doesn't search
...
5. ✅ Review Torrentarr search logs:
   ```bash
...
Ombi has **no built-in rate limiting**, but Torrentarr implements efficient polling:
...
| **Torrentarr Support** | Full support (approved mode only) | Full support (approved/unavailable modes, 4K) |
...
Torrentarr interacts with the following Ombi API endpoints:
...
Torrentarr processes the following fields from Ombi requests:
...
3. **No Release Date Filtering**: Ombi does not provide release dates via API, so Torrentarr cannot skip unreleased content

### Torrentarr Ombi Integration

1. **Approved-Only Mode**: Only filters top-level approval status (not granular for TV episodes)
2. **No Status Updates**: Torrentarr does not update Ombi request status (handled by Ombi ↔ Arr sync)
