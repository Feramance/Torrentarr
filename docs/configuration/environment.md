...
Torrentarr supports environment variable overrides for configuration settings, making it easy to deploy and manage in containerized environments like Docker, Kubernetes, or systemd services.
...
- **Linux:** `~/.local/share/Torrentarr`
- **macOS:** `~/Library/Application Support/Torrentarr`
- **Windows:** `%APPDATA%\Torrentarr`
- **Docker:** `/config`
...
!!! note "Secret Files"
    Torrentarr supports `_FILE` suffix for sensitive variables. The value will be read from the file path specified.
    ...
    spec:
      ...
      - name: config
        persistentVolumeClaim:
          claimName: torrentarr-config-pvc
      - name: downloads
        persistentVolumeClaim:
...
**`/etc/systemd/system/torrentarr.service`:**
...
EnvironmentFile=-/etc/torrentarr/environment
...
3. **Restart required:**
   Changes to environment variables require Torrentarr restart.

4. **Case sensitivity:**
   Variable names are case-sensitive: `QBITRR_SETTINGS_CONSOLE_LEVEL` not `torrentarr_settings_console_level`
...
```bash
docker logs torrentarr | grep -i "config"
```
...
| `QBITRR_SETTINGS_AUTO_UPDATE_ENABLED` | Boolean | `false` | Enable Torrentarr auto-updates |
...
- Restart Torrentarr after changing environment variables
