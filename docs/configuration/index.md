...
Torrentarr's configuration is organized into several logical sections, each controlling different aspects of functionality.
...
These settings are **required** for Torrentarr to function:
...
Configure Torrentarr via environment variables:
...
1. **Generate Default Config**
   ...
   On first run, Torrentarr automatically generates a default configuration file.
...
4. **Restart Torrentarr**
   ...
   After making changes, restart Torrentarr for them to take effect.
...
Follow this recommended workflow when setting up Torrentarr:
...
2. **Generate Default Config**:
   ```bash
   # Docker
   docker start torrentarr  # Stops after generating config
...
After configuring Torrentarr, verify your setup:

### Automated Validation

```bash
# Validate config syntax
torrentarr --validate-config

# Test connections
torrentarr --test-connections
...
- [ ] **Paths Accessible** - Torrentarr can read download paths
...
1. **Check Logs for Connections**:
   ```bash
   docker logs torrentarr | grep "Successfully connected"
   ```
...
**Problem**: Torrentarr doesn't track torrents

**Solution**: Ensure categories match:
1. Torrentarr config: `Category = "radarr"`
2. Radarr download client: Category = `radarr`
3. qBittorrent: Category `radarr` exists

### Performance Issues

**Problem**: Torrentarr uses too much CPU/memory
...
- [Features Overview](../features/index.md) - What Torrentarr can do
- [WebUI Guide](../webui/index.md) - Use the web interface
- [Development Guide](../development/index.md) - Contribute to Torrentarr
