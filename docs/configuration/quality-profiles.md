...
Quality profiles in Radarr, Sonarr, and Lidarr control which media qualities are acceptable and when to upgrade. Torrentarr's temporary quality profile feature allows dynamic profile switching for hard-to-find content.
...
## Torrentarr Temporary Quality Profiles
...
- Torrentarr automatically switches to "Any" profile
...
1. Torrentarr detects missing album/movie/episode
...
6. After import, Torrentarr switches back to main profile
...
**When false (default):**
- Download completes
- Torrentarr switches back to main profile
...
Reset all items using temp profiles to main profiles on Torrentarr startup.

**When true:**
- Torrentarr starts up
...
**When enabled:**
- Torrentarr tracks when temp profile was set
...
sequenceDiagram
    ...
    Lidarr->>Torrentarr: Notify Missing Album
    Torrentarr->>Indexer: Search for FLAC
    Indexer-->>Torrentarr: No FLAC Found

    Note over Torrentarr: Switch to Temp Profile
    Torrentarr->>Lidarr: Update Profile<br/>→ Lossy (MP3-320)
    Torrentarr->>Indexer: Search for MP3
    Indexer-->>Torrentarr: MP3-320 Available
    Torrentarr->>Lidarr: Download MP3-320

    Lidarr->>Torrentarr: Import Complete
    Note over Torrentarr: Switch Back to Main
    Torrentarr->>Lidarr: Update Profile<br/>→ Lossless (FLAC)

    loop Upgrade Search
        Torrentarr->>Indexer: Check for FLAC
        Indexer-->>Torrentarr: FLAC Available!
        Torrentarr->>Lidarr: Download & Replace
    end
```
...
stateDiagram-v2
    ...
    Missing --> SearchingUHD: Torrentarr Searches<br/>for 4K
