...
- **Disabled**: Disable Torrentarr's qBittorrent integration (headless search-only mode)
...
- **Managed**: Toggle whether Torrentarr actively manages this instance
...
- **Re-search**: Re-run searches for failed torrents after Torrentarr removes them
...
Configure how Torrentarr evaluates and manages torrents.
...
10. Torrentarr begins polling Ombi for pending requests
...
2. **Permission Issues**: Torrentarr cannot read `config.toml`
...
- Regenerate token: Remove `WebUI.Token` from config, restart Torrentarr
...
2. **Write Permissions**: Torrentarr cannot write to `config.toml`
...
- Restart Torrentarr: `systemctl restart torrentarr` (systemd) or restart Docker container
