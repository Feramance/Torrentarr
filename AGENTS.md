...
```
The Host serves the UI from `src/Torrentarr.Host/wwwroot/`; that directory is build output (generated by `npm run build` in webui/) and is not committed. CI and Docker run the frontend build as part of the pipeline.
...
**Config version:** Current `Settings.ConfigVersion` / `ConfigurationLoader.ExpectedConfigVersion` is **`6.14.4`**. That value is the **Torrentarr** schema id — it follows the **+1 major vs qBitrr** release policy (see **What This Project Is** above), not qBitrr’s raw package version. Notable fields: `HitAndRunMode` is a **string** (`"and"` / `"or"` / `"disabled"`) — not a boolean. Seeding configuration (`HitAndRunMode`, `MinSeedRatio`, `MinSeedingTimeDays`, etc.) lives in `[qBit.CategorySeeding]` per qBit instance — not in `[WebUI]`. `Settings.AutoUpdateChannel` is `"latest"` / `"stable"` / `"nightly"`. Arr sections include `[Readarr-*]`.
