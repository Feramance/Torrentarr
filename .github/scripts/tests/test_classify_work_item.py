...
POLICY = json.loads(
    (
        Path(__file__).parents[2] / "automation" / "classification-policy.json"
    ).read_text()
)


def pull(
    title: str, author: str = "human", body: str = "", head: str = "topic"
) -> dict:
    return {
        "number": 10,
        "title": title,
...
def changed(*paths: str) -> list[dict]:
    return [
        {"filename": path, "status": "modified", "additions": 1, "deletions": 1}
        for path in paths
    ]
...
class DigestTests(unittest.TestCase):
    ...
    def test_contract_round_trip(self) -> None:
        ...
        self.assertEqual(
            classifier.decode_contract(classifier.encode_contract(value)), value
        )
    ...
    def test_material_human_scope_comments_are_digest_bound(self) -> None:
        ...
        self.assertEqual(
            value["scope_comments"], [{"id": 3, "body": "Scope: add beta"}]
        )
    ...
    def test_pr_digest_changes_with_current_base_or_head(self) -> None:
        ...
        first = classifier.digest(
            classifier.pr_digest_input(
                "o/r", value, changed("src/App.cs"), [], [], "p" * 64, "a" * 40
            )
        )
        second = classifier.digest(
            classifier.pr_digest_input(
                "o/r", value, changed("src/App.cs"), [], [], "p" * 64, "b" * 40
            )
        )
        value["head"]["sha"] = "c" * 40
...
class IssueTests(unittest.TestCase):
    ...
    def test_issue_forms(self) -> None:
        ...
        for expected, body, labels in cases:
            with self.subTest(expected=expected):
                self.assertEqual(
                    classifier.classify_issue(self.issue("x", body, labels))[0],
                    expected,
                )
    ...
    def test_repository_change_categories(self) -> None:
        for rendered, expected in [
            ...
            self.assertEqual(
                classifier.classify_issue(self.issue("x", body, []))[0], expected
            )
...
class PullRequestTests(unittest.TestCase):
    ...
    def test_human_dependency_needs_label(self) -> None:
        ...
        self.assertEqual(
            self.classify(pull("bump"), files, ["dependencies"]), "package_update"
        )
    ...
    def test_exclusive_paths(self) -> None:
        self.assertEqual(
            self.classify(pull("update"), changed("docs/index.md")), "docs"
        )
        self.assertEqual(
            self.classify(pull("update"), changed("tests/test_a.py")), "test"
        )
        self.assertEqual(
            self.classify(pull("update"), changed(".github/workflows/ci.yml")), "ci"
        )

    def test_title_only_fix_is_ambiguous(self) -> None:
        self.assertEqual(
            self.classify(pull("fix: claim"), changed("qBitrr/main.py")), "ambiguous"
        )

    def test_linked_bug_and_feature(self) -> None:
        files = changed("qBitrr/main.py")
        self.assertEqual(
            self.classify(pull("x"), files, links=[{"number": 1, "category": "bug"}]),
            "bug_fix",
        )
        self.assertEqual(
            self.classify(
                pull("x"), files, links=[{"number": 1, "category": "feature"}]
            ),
            "feature",
        )
    ...
    def test_managed_handoff_requires_all_signals(self) -> None:
        ...
        self.assertEqual(
            self.classify(
                value,
                changed("qBitrr/main.py"),
                links=[{"number": 1, "category": "bug"}],
            ),
            "bug_fix",
        )
    ...
    def test_flags(self) -> None:
        ...
        files = changed(
            ...
            "src/Torrentarr/Security/Migrations/001.cs",
    ...
    def test_every_pr_category_has_a_conservative_route(self) -> None:
        source = changed("src/Torrentarr/App.cs")
        cases = [
            (
                "package_update",
            ...
            ("test", pull("update"), changed("tests/App.Tests/Test.cs"), [], []),
            ("ci", pull("update"), changed(".github/workflows/build.yml"), [], []),
            ("maintenance", pull("update"), changed("scripts/check.sh"), [], []),
    ...
    def test_failed_pr_classification_clears_ready_and_finishes_check(self) -> None:
        class FailingAPI:
            ...
            def request(self, method: str, path: str, payload=None):
                ...
                if (method, path) == ("POST", "/check-runs"):
                    return {"id": 123}
                if (method, path) == ("GET", "/git/ref/heads/master"):
        ...
        with patch.object(
            classifier, "update_classification_labels"
        ) as update_labels, patch.object(classifier, "finish_check") as finish:
...
class WorkflowSecurityTests(unittest.TestCase):
    def test_pr_classifier_uses_only_immutable_base_sha(self) -> None:
        workflow = (
            Path(__file__).parents[2] / "workflows" / "classify-prs.yml"
        ).read_text()
    ...
    def test_material_issue_comments_trigger_reclassification(self) -> None:
        workflow = (
            Path(__file__).parents[2] / "workflows" / "classify-issues.yml"
        ).read_text()
