...
def material_scope_comments(
    comments: Sequence[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    """Select human comments that explicitly change scope or acceptance criteria."""
    selected: list[dict[str, Any]] = []
    marker = re.compile(
        r"(?im)^\s*(scope|requirements?|acceptance criteria|updated scope)\s*:"
    )
...
def classify_issue(
    issue: Mapping[str, Any], override: str | None = None
) -> tuple[str, list[str]]:
...
def _exclusive_path_category(
    paths: Sequence[str], policy: Mapping[str, Any]
) -> str | None:
    categories = (
    ...
    matches = [
        category for category, key in categories if all_paths_match(paths, policy[key])
    ]
...
def dependency_only(
    ...
    patterns = list(policy["dependency_files"]) + list(
        policy["dependency_companion_files"]
    )
    workflow_files = [
        item
...
def classify_pr(
    ...
    conventional = re.match(
        r"^([a-z]+)(?:\([^)]*\))?!?:", str(pull.get("title") or ""), re.I
    )
    conventional_map = {
        "feat": "feature",
        "docs": "docs",
...
def authorized_override(
    ...
    matching = [
        ...
        and (event.get("label") or {}).get("name") == target
    ]
    if not matching:
        return None
    actor = str((matching[-1].get("actor") or {}).get("login") or "")
    permission = api.request(
        "GET", f"/collaborators/{urllib.parse.quote(actor)}/permission"
    )
...
def current_issue_classification(api: GitHubAPI, number: int) -> dict[str, Any] | None:
    ...
    if len(categories) == 1 and "automation/classification:ready" in labels_from_item(
        issue
    ):
...
def ensure_labels(api: GitHubAPI) -> None:
    ...
    definitions = {
        ...
        "automation/classification:ready": (
            "0E8A16",
            "Classification contract is current",
        ),
    }
...
def update_classification_labels(
    ...
    retained = [
        ...
        and label
        not in {"automation/classification:pending", "automation/classification:ready"}
    ]
...
def upsert_contract_comment(
    api: GitHubAPI, number: int, contract: Mapping[str, Any]
) -> None:
...
def finish_check(
    api: GitHubAPI, check_id: int, conclusion: str, title: str, summary: str
) -> None:
...
def classify_live_pr(api: GitHubAPI, number: int, policy: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        api.request(
            "PATCH",
            f"/check-runs/{check_id}",
            {"external_id": f"{CLASSIFIER_VERSION}:{source}"},
        )
        finish_check(
            api, check_id, "success", f"Classified as {category}", compact_json(result)
        )
    except Exception as error:
        update_classification_labels(
            api, number, labels_from_item(pull), "ambiguous", False
        )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--policy", default=".github/automation/classification-policy.json"
    )
