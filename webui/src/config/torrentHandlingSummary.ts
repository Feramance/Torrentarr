...
export function getArrTorrentHandlingSummary(
  state: ConfigDocument | null,
): string {
  ...
  const ignoreYounger = parseDurationToSeconds(
  ...
  if (Number.isFinite(maxDeletable) && maxDeletable <= 100) {
    newParts.push(
      `torrents at or below ${maxDeletable}% completion may be removed`,
    );
  }
  newParts.push(
    doNotRemoveSlow
      ? "slow downloads are kept"
      : "slow downloads may be removed",
  );
  newParts.push(
    autoDelete
      ? "non-media files are auto-deleted"
      : "non-media files are kept",
  );
  const matchLine =
    (caseSensitive ? "Case-sensitive" : "Case-insensitive") +
    (folderExcl || fileExcl
      ? ` matching with ${folderExcl + fileExcl} exclusion(s)`
      : "") +
  ...
  if (!Number.isFinite(stalledDelayMin) || stalledDelayMin < 0) {
    ...
    blocks.push(
      `If the download stops progressing for ${formatMinutes(stalledDelayMin)}, it is treated as stalled and will be removed after that delay. ` +
        `Before removing, ${reSearchStalled ? "a replacement is searched for." : "no replacement search is performed."}`,
    );
  }

  // 3. When seeding
  const seeding = getVal<Record<string, unknown>>(
    state,
    ["Torrent", "SeedingMode"],
    {},
  );
  ...
  if (Number.isFinite(maxRatio) && maxRatio >= 0) {
    seedParts.push(
      `Maximum ratio is ${maxRatio} (upload ${maxRatio}\u00d7 what you downloaded).`,
    );
  }
  ...
  seedParts.push(
    removeDead ? "Dead trackers are removed." : "Dead trackers are kept.",
  );
  if (removeMessages > 0) {
  ...
  if (hnrMode === "disabled") {
    seedParts.push(
      "Hit and Run is off, so all torrents follow these seeding and stalled rules only.",
    );
  } else {
    const daysVal = Number.isFinite(minDays) ? minDays : 0;
    const daysLabel = plural(
      daysVal === 1 ? 1 : Math.max(0, daysVal),
      "day",
      "days",
    );
    const req =
  ...
  const trackers = get(state, ["Torrent", "Trackers"]) as
    | ConfigDocument[]
    | undefined;
  if (!Array.isArray(trackers) || trackers.length === 0) {
    ...
    trackers.forEach((raw) => {
      ...
      if (mode === "disabled") {
        blocks.push(
          `- **${name}** \u2014 HnR is off. The torrent may be removed as soon as the seeding rules above are met.${sortNote}`,
        );
      } else {
        const both = mode === "and" ? "both required" : "either allows removal";
        blocks.push(
          `- **${name}** \u2014 HnR is on (${both}). The torrent will **not** be removed until it has reached ratio ${ratioStr} and been seeding for ${daysVal} ${daysLabel}. Until then, it is protected from removal even if stalled or if the global seeding limit would allow removal.${sortNote}`,
        );
      }
    });
    blocks.push(
      "Other trackers use qBit instance defaults and the seeding/stalled rules above.",
    );
  }
...
export function getQbitTorrentHandlingSummary(
  state: ConfigDocument | null,
): string {
  if (!state || typeof state !== "object") {
    return "No torrent handling rules configured.";
  ...
  const ignoreYounger = parseDurationToSeconds(
    seeding.IgnoreTorrentsYoungerThan,
    -1,
  );
  ...
  seedParts.push(
    dlLimit >= 0 || ulLimit >= 0
      ? "Rate limits are set (per-torrent speed caps)."
      : "Rate limits are off.",
  );
  seedParts.push(
    "Torrents are not removed for being slow or stalled except as above.",
  );
  blocks.push(seedParts.join(" "));
  ...
  if (hnrMode === "disabled") {
    blocks.push(
      "Hit and Run is off at the category level, so all torrents follow these seeding and stalled rules only.",
    );
  } else {
    const daysVal = Number.isFinite(minDays) ? minDays : 0;
    const daysLabel = plural(
  ...
  if (!Array.isArray(trackers) || trackers.length === 0) {
    ...
    trackers.forEach((raw) => {
      ...
      if (mode === "disabled") {
        blocks.push(
          `- **${name}** \u2014 HnR is off. The torrent may be removed as soon as the seeding rules above are met (e.g. after max time or when max ratio is reached).${sortNote}`,
        );
      } else {
        const both = mode === "and" ? "both required" : "either allows removal";
        const until =
        ...
        blocks.push(
          `- **${name}** \u2014 HnR is on (${both}). The torrent will **not** be removed ${until}. Until then, it is protected from removal even if stalled or if the global seeding time would allow removal.${sortNote}`,
        );
      }
    });
    blocks.push(
      "Other trackers use the category defaults and the seeding/stalled rules above.",
    );
  }
